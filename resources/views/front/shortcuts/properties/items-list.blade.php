                ...
                <div class="h-full md:flex">
                    <div class="relative overflow-hidden md:shrink-0">
                        <a href="{{ route('public.property_single', ['uid' => $property->unique_id, 'slug' => $property->slug ]) }}">
                            <img loading="lazy" class="object-cover w-full h-full transition-all duration-500 md:w-48 hover:scale-110" src="{{ RvMedia::getImageUrl($property->image, 'small', false, RvMedia::getDefaultImage()) }}" alt="{{ $property->name }}">
                        </a>
                    ...
                    <div class="p-6">
                        <div>
                            <div class="hidden md:block -ms-0.5 mb-2">
                                <a href="{{ $property->category->slug }}" class="text-sm transition-all hover:text-primary">
                                    <i class="mdi mdi-tag-outline"></i>
                                    {{ $property->category->name }}
                                </a>
                            </div>
                            <a href="{{ route('public.property_single', ['uid' => $property->unique_id, 'slug' => $property->slug ]) }}" class="text-lg font-medium duration-500 ease-in-out hover:text-primary" title="{{ $property->name }}">
                                {{ $property->name }}
                            </a>
                            @if($property->city || $property->state->name)
                                <p class="truncate text-slate-600 dark:text-slate-300">{{ $property->city ? $property->city  : '' }}{{-- . ', ' . $property->state->name --}}</p>
                            @else
                        ...
                        <ul class="flex flex-wrap gap-3 items-center justify-between pt-4 ps-0 mb-0 list-none md:pt-4">
                            <li>
                                ...
                                <p class="text-lg font-medium">{{-- format_price($property->price, $property->currency) --}} {{ shorten_price($property->price) }}</p>
                            </li>
                            <li>
