        ...
        <div
            ...
            <nav>
                <ol class="flex flex-wrap items-center gap-2">
                    ...
                    <li>
                        <a class="flex items-center gap-3 font-medium" href="{{ route('admin.blogs-category.index') }}">
                            ...
                            <span class="hover:text-primary">Blog Post</span>
                        </a>
                    </li>
    ...
    <div class="flex flex-col gap-9">
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            <form enctype="multipart/form-data"
                action="{{ isset($post) ? route('admin.blogs.update', $post->id) : route('admin.blogs.store') }}"
                method="POST">
                @csrf
                @if (isset($post))
                    @method('PUT')
                ...
                <div class="p-6.5 ">
                    ...
                    <div class="mb-5 w-full">
                        <label class="block text-sm font-medium text-black dark:text-dark">Image</label>
                        <div x-data="imageHandler('{{ isset($post) && $post->image ? asset('images/' . $post->image) : '' }}')" class="mt-2 flex gap-15">
                    ...
                    <div class="mb-5 w-1/2">
                        <label class="block mb-2 text-sm font-medium text-black dark:text-dark">Name</label>
                        <input type="text" name="name" required class="form-control"
                            value="{{ old('name', $post->title ?? '') }}" />
                    </div>

                    <div class="mb-5 w-1/2">
                        <label class="block mb-2 text-sm font-medium text-black dark:text-dark">Category</label>
                        <select name="category" required class="form-control text-capitalize">
                            @foreach ($categories as $category)
                                <option {{ (isset($post) && $post->id == $category->id) ? 'selected' : '' }} class="text-capitalize" value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="mb-5 w-full">
                        <label class="block mb-2 text-sm font-medium text-black dark:text-dark">Description</label>
                        <input type="text" name="description" class="form-control tinyeditor"
                        ...
                        <div class="form-group mb-3 col-md-4">
                            <label class="form-check form-switch">
                                <input class="form-check-input" name="status" type="checkbox" value="1"
                                    id="status"
                                    {{ old('status', isset($post) && $post->status ? 'checked' : '') }}>
