            ...
            <ol class="flex flex-wrap items-center gap-2">
                ...
                <li>
                    <a class="flex items-center gap-3 font-medium" href="{{ route('admin.blogs-category.index') }}">
                      ...
                      <span class="hover:text-primary">Blog Category</span>
                    </a>
                  </li>
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div class="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                <h2 class="text-title-sm2 font-bold text-black dark:text-black">
                    {{ isset($category) ? 'Edit blog category' : 'Create a blog category' }}
                </h2>
            </div>

            <!-- Form -->
            <form enctype="multipart/form-data"
                action="{{ isset($category) ? route('admin.blogs-category.update', $category->id) : route('admin.blogs-category.store') }}"
                method="POST">
                @csrf
                @if (isset($category))
                    @method('PUT')
                ...
                <div class="p-6.5 ">
                    ...
                    <div class="mb-5 w-full">
                        ...
                        <div x-data="imageHandler('{{ isset($category) && $category->image ? asset('images/' . $category->image) : '' }}')" class="mt-2 flex gap-15">
                            ...
                            <label for="image"
                                ...
                                <span class="small">Recommended resolution 670*1000 with Max size 100kb</span>
                                <small></small>
                    ...
                    <div class="mb-5 w-1/2">
                        <label class="block text-sm font-medium text-black dark:text-dark">Name</label>
                        <input type="text" autocomplete="off" name="name" required class="form-control" value="{{ old('name', $category->name ?? '') }}" />
                    </div>
                    ...
                    <div class="mb-5 w-1/4">
                        ...
                        <select class="form-control mt-2" name="display_order" required>
                            @for ($i = 1; $i < 10; $i++)
                                <option {{ isset($category) && $category->display_order == $i ? 'selected' : '' }}
                                    value="{{ $i }}">{{ $i }}</option>
                            @endfor
                        </select>
                    </div>
