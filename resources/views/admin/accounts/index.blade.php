    ...
    <div>
        ...
        <div class="col-span-12">
            <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                ...
                <div x-data="{ activeTab: '{{ request()->status ?? 'pending' }}' }">
                    <!-- ===== Consults List Start ===== -->
                    <div class="col-span-12">
                        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                            <div class="col-span-12 px-4 mt-3">
                                <nav>
                                    <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                        <button
                                            ...
                                            @click="activeTab = 'pending'; window.history.replaceState(null, '', window.location.pathname + '?status=pending')"
                                            class="nav-link"
                                            type="button"
                                            role="tab"
                                        >Pending</button>
                                        <button
                                            ...
                                            @click="activeTab = 'approved'; window.history.replaceState(null, '', window.location.pathname + '?status=approved')"
                                            class="nav-link"
                                            type="button"
                                            role="tab"
                                        >Approved</button>
                                        <button
                                            ...
                                            @click="activeTab = 'suspended'; window.history.replaceState(null, '', window.location.pathname + '?status=suspended')"
                                            class="nav-link"
                                            type="button"
                                            role="tab"
                                        >Suspended</button>
                                ...
                                <div class="tab-content" id="nav-tabContent">
                                    <!-- pending Tab -->
                                    <div x-show="activeTab === 'pending'" class="tab-pane fade show active" id="nav-pending"
                                        aria-labelledby="nav-pending-tab">
                                        @include('admin.accounts.table', ['accounts' => $pending_accounts,'activeTab' => 'pending'])
                                    </div>
            
                                    <!-- approved Tab -->
                                    <div x-show="activeTab === 'approved'" class="tab-pane show active" id="nav-approved"
                                        aria-labelledby="nav-approved-tab">
                                        @include('admin.accounts.table', ['accounts' => $approved_accounts,'activeTab' => 'attended'])
                                    </div>

                                    <!-- Suspended Tab -->
                                    <div x-show="activeTab === 'suspended'" class="tab-pane show active" id="nav-suspended"
                                        aria-labelledby="nav-suspended-tab">
                                        @include('admin.accounts.table', ['accounts' => $suspended_accounts,'activeTab' => 'suspended'])
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <!-- Modal -->
        <div id="account-modal" class="hidden fixed inset-0 z-9999 bg-black bg-opacity-50 flex justify-center items-center">
