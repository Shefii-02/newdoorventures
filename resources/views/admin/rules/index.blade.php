                            ...
                            <div class="flex justify-between items-center gap-3">
                                <div class="w-2/12 xl:w-3/12">
                                    <div class="flex items-center gap-4">
                                        <img src="{{ $rule->image_url }}" style="width: 35px">
                                        <span class="hidden font-medium xl:block">{{ $rule->name }}</span>
                                    </div>
                                </div>
                                <div class="w-4/12 2xsm:w-3/12 md:w-2/12 xl:w-1/12">
                                    <span
                                        class="inline-block text-capitalize rounded {{ $rule->status == 'pending' ? 'bg-red' : 'bg-success' }} px-2.5 py-0.5 text-sm font-medium text-white">
                                        {{ $rule->status }}
                                    </span>
                                </div>
                                <div class=" w-2/12 2xsm:block md:w-1/12">
                                    <div class="flex justify-between">
                                        <a href="{{ route('admin.rules.edit',$rule->id) }}" class="mx-auto block hover:text-meta-1">
                                        ...
                                        <form method="POST" id="form_{{ $rule->id }}" action="{{ route('admin.rules.destroy',$rule->id) }}">@csrf @method('DELETE')</form>
                                        <button form="form_{{ $rule->id }}" type="submit" class="mx-auto block hover:text-meta-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
