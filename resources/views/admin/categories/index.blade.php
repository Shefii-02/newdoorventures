                            ...
                            <div class="flex justify-between items-center gap-3">
                                <div class="w-2/12 xl:w-3/12">
                                    <div class="flex items-center gap-4">
                                        <span class="hidden font-medium xl:block">{{ $category->name }}</span>
                                    </div>
                                </div>
                                <div class="w-4/12 2xsm:w-3/12 md:w-2/12 xl:w-1/12">
                                    <span
                                        class="inline-block text-capitalize rounded {{ $category->status == 'pending' ? 'bg-red' : 'bg-success' }} px-2.5 py-0.5 text-sm font-medium text-white">
                                        {{ $category->status }}
                                    </span>
                                </div>
                                <div class=" w-2/12 2xsm:block md:w-1/12">
                                    <div class="flex justify-between">
                                        <a href="{{ route('admin.categories.edit',$category->id) }}" class="mx-auto block hover:text-meta-1">
                                        ...
                                        <form method="POST" id="form_{{ $category->id }}" action="{{ route('admin.categories.destroy',$category->id) }}">@csrf @method('DELETE')</form>
                                        <button form="form_{{ $category->id }}" type="submit" class="mx-auto block hover:text-meta-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
