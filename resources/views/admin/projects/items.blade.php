@foreach ($projects as $project)
    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
        <td class="w-4 p-4 " colspan="3">
            <a href="#" role="button" title="{{ $project->name }}">
                <img src="{{ asset('images/' . $project->image) }}" class="rounded mb-2 text-center" width="50"
                    alt="Image">
                <span
                    class="fw-bold text-left text-dark-emphasis ">{{ Str::limit($project->name, '15', '...') }}</span>
            </a>
        </td>
        ...
        <td class="px-2 py-1 text-end">
            ...
            <form method="POST" id="form_{{ $project->id }}" action="{{ route('admin.projects.destroy',$project->id) }}">@csrf @method('DELETE')</form>
            <button form="form_{{ $project->id }}" type="submit" class="mx-auto block hover:text-meta-1 ms-3">
