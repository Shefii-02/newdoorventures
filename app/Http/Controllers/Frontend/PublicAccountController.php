...
class PublicAccountController extends BaseController
{
    ...
    if(auth('account')->user()->status == 'approved'){
            return redirect()->route('public.account.dashboard');
    }

        $this->pageTitle('Account pending stage');
        session()->flash('notification', 'Your account is pending for approval. Please wait or call the support center.');
        $user = auth('account')->user();
        return view('plugins/real-estate::themes.dashboard.notification', compact('user'));
    }
