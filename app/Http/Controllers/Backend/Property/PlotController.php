...
use Botble\RealEstate\Forms\Property\PlotForm as PropertyForm;
...
use Botble\RealEstate\Tables\Property\PlotTable as PropertyTable;
use Carbon\Carbon;
use Exception;
...
class PlotController extends BaseController
{
    public function __construct()
    {
        ...
        $this
            ->breadcrumb()
            ->add(trans('plugins/real-estate::property.plot.name'), route('property.plot.index'));
    }

    public function index(PropertyTable $dataTable)
    {
        $this->pageTitle(trans('plugins/real-estate::property.plot.name'));
    ...
    {
        $this->pageTitle(trans('plugins/real-estate::property.plot.create'));
    ...
    public function store(
        ...
        return $this
            ->httpResponse()
            ->setPreviousUrl(route('property.plot.index'))
            ->setNextUrl(route('property.plot.edit', $property->id))
    ...
    public function edit(int|string $id, Request $request)
    {
        ...
        $this->pageTitle(trans('plugins/real-estate::property.plot.edit') . ' "' . $property->name . '"');
